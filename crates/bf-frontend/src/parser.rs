use crate::{
    ast::{AST, ASTNode, ASTNodeKind, InvalidNode, Span},
...
impl Parser<'_> {
    ...
    fn parse_loop(&mut self, start_loc: usize) -> ASTNode {
        ...
        if is_closed {
            ASTNode {
                kind: ASTNodeKind::Loop { body },
                span,
            }
        } else {
            ASTNode {
                kind: ASTNodeKind::Invalid(InvalidNode::OpenLoopStart),
                span,
            }
        }
    }
    ...
    pub fn parse(mut self) -> AST {
        ...
        while let Some(t) = self.peek() {
            ...
            let n = match t.kind() {
                ...
                TokenKind::LoopEnd => {
                    self.parse_one(loc, ASTNodeKind::Invalid(InvalidNode::OpenLoopEnd))
                }
            };
