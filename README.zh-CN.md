...
内部 crate 需要按照依赖顺序发布。首次发布时，下游包的 dry run 需要等上游包已经发布并同步到 crates.io 索引后才能通过：

```text
bf-frontend -> bf-ir -> bf-codegen -> bf-cli
```

从 `bf-frontend` 开始，对每个包先执行 dry run，再正式发布，然后继续下一层：
...
在发布前还应确认 workspace 测试和 Clippy 检查全部通过。crates.io 上已经发布的版本无法覆盖，因此每次重新发布都需要更新版本号。
